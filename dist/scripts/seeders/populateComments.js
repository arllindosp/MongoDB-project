...
async function atribuirComentariosCursos() {
    ...
    for (const comentario of comentariosCursos) {
        ...
        const usuario = await User_1.default.findOne({ name: comentario.userName });
        ...
        if (usuario && curso) {
            ...
            await Course.updateOne({ _id: curso._id }, { $push: { comments: novoComentario._id } });
        }
    }
...
async function atribuirComentariosAulas() {
    ...
    for (const comentario of comentariosAulas) {
        ...
        const aula = await Lesson_1.default.findOne({ title: comentario.aula });
        if (usuario && aula) {
            // Insere o comentário na collection
            const novoComentario = await Comment.create({
                userId: usuario._id,
            ...
            await Lesson_1.default.updateOne({ _id: aula._id }, { $push: { comments: novoComentario._id } });
        }
    }
...
async function main() {
    await mongo_1.default.connection.dropCollection("comments").catch(() => { });
