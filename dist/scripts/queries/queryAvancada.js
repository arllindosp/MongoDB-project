...
async function queryAvancada() {
    ...
    const categoriasExatasDocs = await Category_1.default.find({
        name: { $in: ["Ciências Exatas e Engenharia", "Matemática e Estatística"] },
    });
    const categoriasExatasIds = categoriasExatasDocs.map((cat) => cat._id);
    // Encontrar estudantes matriculados em cursos de exatas (usando $lookup e $match)
    const estudantesExatas = await Student_1.default.aggregate([
        {
            $lookup: {
                from: "courses",
    ...
    const cursosAvancados = await Course_1.default.aggregate([
        {
            $project: {
                title: 1,
                isAvancado: {
    ...
    console.log("No shell do MongoDB, o comando .pretty() apenas formata a saída para facilitar a leitura. Exemplo: db.courses.find().pretty(). No Node.js/Mongoose, não é necessário, pois o resultado já é um objeto JavaScript.");
    console.log("========== Query Avançada ==========");
    console.log(`Estudantes com progresso registrado: ${estudantesComProgresso}`);
    console.log(`Maior número de cursos por estudante: ${maxCursos[0]?.maxCursos ?? "N/A"}`);
    console.log(`Total de estudantes matriculados: ${totalEstudantesMatriculados}`);
    console.log(`Total de alunos com certificados: ${alunosComCertificados}`);
    console.log("Cursos por área:");
    cursosPorArea.forEach((area) => {
        console.log(`  ${area.categoryName ?? area._id}: ${area.total}`);
    });
    console.log("---------------------------------------------\n");
    // Printar alunos em exatas no final
    console.log(`Estudantes matriculados em cursos de exatas: ${estudantesExatas.length}`);
    estudantesExatas.forEach((student) => {
        console.log(`Nome: ${student.name}`);
        console.log("Cursos de exatas:");
        if (student.exatasCourseNames && student.exatasCourseNames.length > 0) {
            student.exatasCourseNames.forEach((courseName) => {
