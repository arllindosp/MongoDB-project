...
async function populatePlaylists() {
  await mongoose.connection.dropCollection("playlists").catch(() => {});
  ...
  for (const playlist of playlists) {
    ...
    const instructor = user
      ? await Instructor.findOne({ userId: user._id })
      : null;
    if (!instructor && user) {
      console.log(
        `Instrutor não encontrado para usuário: ${playlist.instructorName}`
      );
    }
    ...
    if (instructor && course) {
      ...
      await mongoose.connection
        ...
        .updateOne(
          { _id: course._id },
          { $addToSet: { playlistId: createdPlaylist._id } }
        );
    }
  }
